...
function installFloatingWindows(mainWindow, trusted) {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url, frameName }) => {
    ...
    return {
      action: 'allow', overrideBrowserWindowOptions: {
        ...
        alwaysOnTop: false, resizable: true, maximizable: false, minimizable: false,
  ...
  ipcMain.handle('roomcast:floating-action', (event, { id, action } = {}) => {
    ...
    if (action === 'close') child.close();
      ...
      child.setAlwaysOnTop(!child.isAlwaysOnTop(), 'floating');
      notify(id, child.isFullScreen() ? 'FLOATING_FULLSCREEN' : 'FLOATING');
    }
