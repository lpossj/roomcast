# Roomcast 0.14.2 Beta：EXE 使用与异地开房
...
2. 点击 **邀请朋友**，复制完整邀请链接。同一个弹窗里还会自动创建"电脑／手机网页观看链接"，接收方不装 Roomcast 也能用浏览器打开。
3. 好友有 Roomcast 时先运行程序再打开邀请链接；若聊天软件不能打开自定义链接，可在 **加入房间** 中粘贴完整邀请。没有安装的好友直接打开网页观看链接即可，手机浏览器以观看和聊天为主。
4. 房主离开时，现有房间迁移逻辑按程序当前规则处理；网页入口会在离开房间或退出应用时关闭。
...
旧 Quick Tunnel / MediaMTX **媒体链**已经退役，不应作为当前故障排查方案恢复。`cloudflared` 现在只用于"分享房间"里的临时静态网页入口，不承载媒体。

## 常见问题

| 情况 | 处理方法 |
| --- | --- |
| 邀请链接打不开 | 有 Roomcast 时先运行程序再打开，或在"加入房间"中粘贴完整邀请；没安装时用"分享房间"里的网页观看链接。 |
| 网页观看链接打不开或一会儿就失效 | 网页入口基于 Cloudflare Quick Tunnel，地址可能变化且不保证可用性；请分享者保持 Roomcast 运行并重新复制链接。 |
| 手机网页点共享没反应 | 手机浏览器通常不实现 `getDisplayMedia`，网页端手机只能观看和聊天。 |
