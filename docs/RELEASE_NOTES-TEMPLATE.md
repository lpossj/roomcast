...
- 桌面端需要安装 Roomcast；接收方也可以不安装，用浏览器打开"分享房间"里的网页观看链接。
- 手机浏览器通常不实现 `getDisplayMedia`，网页端手机只能观看和聊天，无法发起屏幕共享。
- 网页入口基于 Cloudflare Quick Tunnel（测试用途、地址可能变化、有并发限制），分享者需保持应用或页面运行。
