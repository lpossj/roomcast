...
- [x] **真实网络下载**：用线上 `v0.14.2-beta.2` 的 160 KB 资产跑通"真实资产地址 → 下载 → 拉取该 release 的真实 `SHA256.txt` → 解析 → 哈希比对"，结果 `verified: true`，哈希与本地打包的同一资产一致
- [x] **失败态 UI**（真实浏览器 + stub 桌面桥）：检查超时 → 高亮失败块 + 手动按钮由 `subtle` 变 `secondary` 且仍可点；发现新版本 → 版本号/测试版标注/提示点/两个产物按钮（含体积）；下载超时 → 独立失败块且按钮可重试；点击手动下载 → 成功调起；已是最新 → 干净状态行、无提示点
- [x] 无重复显示：错误信息只在专门的高亮块里出现一次
- [x] **打包后端到端**：真实便携版里 4 个更新 API 暴露正常，"设置 → 关于"出现"软件更新"，显示当前版本 `0.14.2-beta.9`，点击"检查更新"走通 IPC + Chromium 网络栈并返回"已是最新版本"
