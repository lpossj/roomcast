...
- **Release 工作流只把 `Roomcast-<版本>-WebViewer.zip` 作为资产上传，不会自动部署 gh-pages**，
  所以发布当时线上仍是 `0.14.3-beta.2`。
- **16:37 决定（用户）**：不再打包与发布网页包 —— 见 [STEP-13-停用网页包发布.md](STEP-13-停用网页包发布.md)。
- **16:42 手工发布一次**（同时把网页端改成"只加入观看"）—— 见
  [STEP-14-网页端去掉建房入口.md](STEP-14-网页端去掉建房入口.md)：
  gh-pages `fcf7221 → d8b710d`，线上 `version.json` 实测已是 `0.14.3-beta.3`。
  因此本条遗留**已解决**；站点内容此后按 STEP-13 的约定保持冻结，除非再次手工发布。
