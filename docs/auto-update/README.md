...
| 16 | [STEP-16-限流降级改用发布订阅.md](STEP-16-限流降级改用发布订阅.md) | 真实更新测试暴露：限流时退回已冻结的站点清单 → 检查不到新版本；降级链改为"发布订阅优先"，零额度且同源 |
| 17 | [STEP-17-修复替换脚本被主程序连带终止.md](STEP-17-修复替换脚本被主程序连带终止.md) | beta.5 的替换脚本被主程序退出连带杀死（静默失败）→ 改为两级隐藏启动器；真机归档测试加固 |
| 18 | [STEP-18-同号重发0.14.3-beta.5.md](STEP-18-同号重发0.14.3-beta.5.md) | 按用户要求**同号重发**：移动标签 + 重复触发同一标签的 Release 构建覆盖资产（不新增 beta.6）；含代价与收尾核对项 |
