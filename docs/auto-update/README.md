...
| 12 | [STEP-12-发布0.14.3-beta.3.md](STEP-12-发布0.14.3-beta.3.md) | 提交与发布操作的时间戳记录（版本号三处、git 代理坑、发布工作流与 `version.json` 遗留） |
| 13 | [STEP-13-停用网页包发布.md](STEP-13-停用网页包发布.md) | 按用户决定停用网页包（WebViewer）的打包与发布：只删打包脚本与 npm 脚本，保留站点与浏览器观看能力 |
| 14 | [STEP-14-网页端去掉建房入口.md](STEP-14-网页端去掉建房入口.md) | 网页端只保留"邀请链接加入观看 + 房间内浏览器共享"，建房只由桌面端发起；并重新发布站点 |
| 15 | [STEP-15-修复更新期间控制台窗口并精简下载入口.md](STEP-15-修复更新期间控制台窗口并精简下载入口.md) | 修复更新期间弹出的 3 个 cmd 窗口（`DETACHED_PROCESS` 让 `windowsHide` 失效，含对照实验）；删除多余的 EXE/ZIP 手动下载按钮与相关死代码 |
