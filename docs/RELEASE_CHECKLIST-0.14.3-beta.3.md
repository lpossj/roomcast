...
## 发布结果（2026-09-26 16:47）

- Release 工作流 `36230079046`：**completed / success**；发布页
  <https://github.com/lpossj/roomcast/releases/tag/v0.14.3-beta.3>（预发布、非草稿）。
- 资产：Windows EXE（149,916,762 B）、Windows ZIP（221,713,396 B）、source.zip（850,462 B）、
  loopback-capture.zip（163,704 B）、SHA256.txt、OBS-Studio-32.1.2-Sources.tar.gz。
- 发布页 `SHA256.txt` 中每个资产的哈希与 GitHub 对该文件的摘要**逐个一致**（自动更新的校验会通过）。
- 说明：本版**不含** 16:37 / 16:42 的"停用网页包 + 网页端改为只加入观看"改动（那些随下一个版本进入安装包），
  但固定站点已在 16:42 更新，`version.json` 已指向本版。
