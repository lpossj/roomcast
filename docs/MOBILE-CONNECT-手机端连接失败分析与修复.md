...
> 复审更正（2026-09-26 21:08:52 +08:00）：第1–7节包含已撤回的方案及未经证实的根因推断，不作为现行修复说明。最新独立证据见 `docs/HANDOFF-AUDIT-20260926.md`；本次保持连接/TURN策略不变。
...
复审更正：安卓超时是等待控制通道认证完成；首次加入时此处还未发送 `room:join`，不能据此认定已进房、鉴权正常或根因仅为ICE/中继。iOS原始错误码的具体显示来源与失败阶段仍待实际网页证据核实。

**待用户给出的判定（A/B）**：用同一部手机、同一网络再跑一次 `beta.5`（`Downloads\Roomcast-0.14.3-beta.5-Windows`）。
同一失败条件下beta.5也失败，只说明问题并非beta.6独有，不能单独确定根因。用户已确认两版同网均可观看；不同网络的版本对照仍待真机测试。
